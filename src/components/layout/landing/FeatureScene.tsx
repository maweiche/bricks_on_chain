    ...
    return {
      ...
      glassPanels: Array.from({ length: 8 }).map((_, i) => {
        ...
        return (
          <mesh
            key={`glass-${i}`}
            position={[Math.cos(angle) * 0.8, 2, Math.sin(angle) * 0.8]}
            rotation={[0, angle, 0]}
            castShadow
          >
            <planeGeometry args={[0.3, 2]} />
...
function Lightning() {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      if (Math.random() < 0.1) {
        // 10% chance of lightning every interval
        setIsFlashing(true)
      ...
      {Array.from({ length: 20 }).map((_, i) => {
        ...
        return (
          <group key={i} position={[x, 10 + Math.random() * 10, z]}>
            <Cloud opacity={0.8} speed={0.1} segments={20} color="#ffffff" />
          </group>
        )
      })}
    </group>
...
function TokenizationFlow({ theme = 'light' }) {
  ...
  return (
    <group>
      ...
      {/* Info Points */}
      {featurePoints.map((point, index) => (
        <Html key={index} position={point.position} center distanceFactor={15}>
          <div className="w-48 rounded-lg bg-card/80 p-4 shadow-lg backdrop-blur-sm">
            <h3 className="mb-1 text-sm font-bold">{point.title}</h3>
            <p className="text-xs text-muted-foreground">{point.description}</p>
          </div>
        </Html>
      ))}
