...
export function PurchaseDialog({ isOpen, onClose, property }: PurchaseDialogProps) {
  const [fractionCount, setFractionCount] = useState(1);
  const [purchaseStep, setPurchaseStep] = useState<'input' | 'confirm' | 'processing'>('input');
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>
            {purchaseStep === 'processing' ? 'Processing Purchase' : 'Purchase Fractions'}
          </DialogTitle>
          <DialogDescription>
            ...
            >
              <div className="space-y-2">
                ...
                <Input
                  ...
                  max={maxFractions}
                  value={fractionCount}
                  onChange={(e) => setFractionCount(Math.min(Math.max(1, parseInt(e.target.value) || 1), maxFractions))}
                  className="text-lg"
                />
