...
export interface ProposalSlice {
  ...
  createProposal: (proposal: Omit<Proposal, 'id' | 'createdAt' | 'updatedAt' | 'status' | 'votingPower' | 'votes'>) => Promise<void>
  updateProposal: (id: string, updates: Partial<Proposal>) => Promise<void>
  deleteProposal: (id: string) => Promise<void>

  // Renamed vote operations
  submitProposalVote: (proposalId: string, vote: 'for' | 'against', userAddress: string, votingPower: number) => Promise<void> // renamed from vote
  ...
  checkUserVote: (proposalId: string, userAddress: string) => 'for' | 'against' | null
...
export const createProposalSlice: StateCreator<ProposalSlice> = (set, get) => ({
  ...
  updateProposal: async (id, updates) => {
    ...
    try {
      ...
      setProposalError(error instanceof Error ? error.message : 'Failed to update proposal')
      throw error
    } finally {
      setProposalLoading(false)
    }
  ...
  deleteProposal: async (id) => {
    ...
    try {
      ...
      setProposalError(error instanceof Error ? error.message : 'Failed to delete proposal')
      throw error
    } finally {
      setProposalLoading(false)
    }
  ...
  submitProposalVote: async (proposalId, voteType, userAddress, votingPower) => {
    const { setProposalLoading, setProposalError } = get()
    setProposalLoading(true)
    ...
    try {
      ...
      setProposalError(error instanceof Error ? error.message : 'Failed to cast vote')
      throw error
    } finally {
      setProposalLoading(false)
    }
  ...
  removeProposalVote: async (proposalId, userAddress) => {
    ...
    try {
      ...
      setProposalError(error instanceof Error ? error.message : 'Failed to remove vote')
      throw error
    } finally {
      setProposalLoading(false)
    }
  ...
  getFilteredProposals: () => {
    ...
    return proposals.filter(proposal => {
      if (proposalFilters.status && proposal.status !== proposalFilters.status) return false
      if (proposalFilters.type && proposal.type !== proposalFilters.type) return false
      if (proposalFilters.propertyId && proposal.propertyId !== proposalFilters.propertyId) return false
  ...
  getUserVotingPower: async (userAddress: string) => {
    ...
    try {
      ...
      const totalVotingPower = user.investments.reduce((total: any, investment: { fractionCount: any }) => {
      ...
      setProposalError(error instanceof Error ? error.message : 'Failed to calculate voting power')
