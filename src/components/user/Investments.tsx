...
const ConsolidatedInvestments: React.FC<ConsolidatedInvestmentsProps> = ({ investmentDetails, properties }) => {
  // Group investments by property
  const consolidatedInvestments = useMemo(() => {
    const grouped = investmentDetails.reduce((acc: { [key: string]: any }, investment) => {
  ...
  return (
    ...
    >
      {consolidatedInvestments.map((investment, index) => (
          ...
          <CardContent className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
              ...
              <div className="space-y-1">
                ...
                <p className="text-lg font-semibold">{investment.totalFractions}</p>
              </div>
              <div className="space-y-1">
            ...
            <div className="space-y-2">
              ...
              <Progress 
                value={(investment.property.currentFunding / investment.property.fundingGoal) * 100}
                className="h-2"
              />
            </div>
                ...
                <AccordionContent>
                  <div className="space-y-2">
                    {investment.transactions
                      .sort((a: { purchaseDate: string | number | Date; }, b: { purchaseDate: string | number | Date; }) => new Date(b.purchaseDate).getTime() - new Date(a.purchaseDate).getTime())
                      .map((transaction: { id: React.Key | null | undefined; fractionCount: string | number | bigint | boolean | React.ReactElement<any, string | React.JSXElementConstructor<any>> | Iterable<React.ReactNode> | Promise<React.AwaitedReactNode> | null | undefined; amount: { toLocaleString: () => string | number | bigint | boolean | React.ReactElement<any, string | React.JSXElementConstructor<any>> | Iterable<React.ReactNode> | React.ReactPortal | Promise<React.AwaitedReactNode> | null | undefined; }; purchaseDate: string | number | Date; transactionSignature: string | any[]; }) => (
                        <div
                          key={transaction.id}
                          ...
                          <div className="space-y-1">
                            <div className="flex items-center gap-2">
                              <Badge variant="secondary" className="font-mono text-xs">
                                {transaction.fractionCount ?? 0} fraction{Number(transaction.fractionCount ?? 0) > 1 ? 's' : ''}
                              </Badge>
                            ...
                            <p className="text-xs text-muted-foreground">
                              {format(new Date(transaction.purchaseDate), 'MMM dd, yyyy HH:mm')}
                            </p>
                          </div>
                          <Button
                            variant="ghost"
