...
export const createCartSlice: StateCreator<CartSlice> = (set, get) => ({
  ...
  addItem: (item) => set((state) => {
    const existingItem = state.items.find(i => i.propertyId === item.propertyId);
        ...
        items: state.items.map(i => 
          i.propertyId === item.propertyId
            ? {
                ...
                totalAmount: (i.fractionCount + item.fractionCount) * item.pricePerFraction
              }
            : i
        )
      };
  ...
  removeItem: (propertyId) => set((state) => ({
    items: state.items.filter(item => item.propertyId !== propertyId)
  })),

  updateItemCount: (propertyId, fractionCount) => set((state) => ({
